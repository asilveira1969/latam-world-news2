...
function snapshot(database: string) {
  ...
  const counts = Object.fromEntries(tables.map((table) => {
    ...
    return [table, { count: rows.length, hash: hashRows(rows) }];
  }));
  return { schema: { count: schema.length, hash: hashRows(schema) }, tables: counts, editorial: tables.includes("articles") ? {
...
const equal = snapshotsEqual(sourceSnapshot, targetSnapshot);
