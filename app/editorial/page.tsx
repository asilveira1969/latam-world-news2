...
export default async function EditorialPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const response = await listD1InternalArticles({ editorialStatus: "pending_review", editorialReviewStatus: "pending", createdAfter: editorialDayStart(), pageSize: 100 });
  const articles: EditorialInboxArticle[] = response.data.map((raw) => {
    const article = { id: string(raw.id), slug: string(raw.slug), title: string(raw.title), excerpt: string(raw.excerpt), image_url: string(raw.image_url), source_name: string(raw.source_name), source_url: string(raw.source_url), published_at: string(raw.published_at), editorial_format: string(raw.editorial_format) as EditorialInboxArticle["editorial_format"], latamworldnews_summary: string(raw.latamworldnews_summary), editorial_key_takeaway: string(raw.editorial_key_takeaway), editorial_what_to_watch: string(raw.editorial_what_to_watch), editorial_latam_impact: string(raw.editorial_latam_impact), editorial_generated_at: string(raw.editorial_generated_at), editorial_model: string(raw.editorial_model), editorial_input_hash: string(raw.editorial_input_hash) };
